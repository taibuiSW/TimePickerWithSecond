...
package com.funix.prm391x.se00255x.timepickerwithsecond;
...
public class TimePicker extends FrameLayout {
    ...
    public TimePicker(Context context, AttributeSet attrs, int defStyle) {
     ...
     // digits of seconds
        ...
        // am/pm
        mAmPmGroup = (RadioGroup) findViewById(R.id.amPm);
        ...
        // pre-check radio button in 12-hour view
        if (!mIs24HourView) {
            mAmPmGroup.check(mIsAm ? R.id.radio_btn_AM : R.id.radio_btn_PM);
        }

        mAmPmGroup.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(RadioGroup group, @IdRes int checkedId) {
                requestFocus();
                if (checkedId == R.id.radio_btn_AM) {
                    mIsAm = true;
                    mCurrentHour -= (mCurrentHour >= 12 ? 12 : 0);

                } else if (checkedId == R.id.radio_btn_PM) {
                    mIsAm = false;
                    mCurrentHour += (mCurrentHour < 12 ? 12 : 0);
